...
def _run_exit_code(result: RunResult) -> int:
    if result.status == "INVALID":
        return 2
    if result.status in {"BLOCKED", "IMPLEMENTATION_FAILED", "IMPLEMENTATION_TIMED_OUT", "BOOTSTRAP_FAILED", "VALIDATION_FAILED", "REVIEW_BLOCKED", "REVIEW_CHANGES_REQUESTED", "EXACT_HEAD_BLOCKED", "PUBLICATION_BLOCKED", "CLEANUP_INCOMPLETE", "NO_CHANGES_CLEANUP_INCOMPLETE", "CANDIDATE_BLOCKED"}:
        return 1
