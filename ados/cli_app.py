...
def _run_exit_code(result: RunResult) -> int:
    if result.status == "INVALID":
        return 2
    if result.status in {"BLOCKED", "IMPLEMENTATION_FAILED", "IMPLEMENTATION_TIMED_OUT", "BOOTSTRAP_FAILED", "VALIDATION_FAILED", "REVIEW_BLOCKED", "REVIEW_CHANGES_REQUESTED", "EXACT_HEAD_BLOCKED", "PUBLICATION_BLOCKED", "CLEANUP_INCOMPLETE", "NO_CHANGES_CLEANUP_INCOMPLETE", "NO_CHANGES_AMBIGUOUS", "CANDIDATE_BLOCKED"}:
        return 1
    if result.status in {"PLANNED", "READY_FOR_IMPLEMENTATION", "READY_FOR_VALIDATION", "READY_FOR_PUBLICATION", "COMPLETE", "NO_CHANGES"}:
        return 0
    return 1
