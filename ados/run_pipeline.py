...
class RunPipeline:
        ...
        if record.get("status") == "VALIDATION_FAILED":
            ...
            if adoption is not None:
                ...
                if blocked is not None:
                    reopened = self._reopen_exhausted_validation_recovery(config, run_record_path, record, candidate_artifact, validation_artifact, stages)
                    if isinstance(reopened, PipelineOutcome):
                        return reopened
    ...
    def _recover_validation_failure(
        ...
        if round_number > max_rounds:
            violation = _violation(
                ...
                {
                    "max_recovery_rounds": str(max_rounds),
                    "candidate_sha": candidate.candidate_sha,
                    "policy_fingerprint": _policy_fingerprint(config),
                },
            )
    ...
    def _reopen_exhausted_validation_recovery(
        ...
        validation_artifact: Any,
        stages: list[PipelineStage],
    ) -> dict[str, Any] | PipelineOutcome | None:
        block = record.get("validationRecoveryBlock")
        if not isinstance(block, dict) or str(block.get("status", "")) != "BLOCKED":
            return None
        if str(block.get("reasonCode", "")) != "VALIDATION_RECOVERY_MAX_ROUNDS_EXCEEDED":
            return None
        ...
        if exhausted_max is None:
            exhausted_max = config.execution_policy.validation.max_recovery_rounds
        if len(attempts) < exhausted_max:
            return PipelineOutcome(
                "VALIDATION_FAILED",
                tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {"reason": "attempts_not_exhausted"})]),
                record,
                violations=(
                    _violation(
                        "VALIDATION_RECOVERY_REOPEN_NOT_EXHAUSTED",
                        "validation recovery reopen requires exhausted prior recovery attempts",
                        {"attempts": str(len(attempts)), "max_recovery_rounds": str(exhausted_max)},
                    ),
                ),
            )

        current_identity = _ados_runtime_identity()
        current_policy_fingerprint = _policy_fingerprint(config)
        previous_identity = block.get("recoveryEngine")
        previous_policy_fingerprint = str(block.get("policyFingerprint", ""))
        legacy_block = not isinstance(previous_identity, dict)
        engine_changed = isinstance(previous_identity, dict) and previous_identity.get("gitHead") != current_identity.get("gitHead")
        policy_changed = bool(previous_policy_fingerprint and previous_policy_fingerprint != current_policy_fingerprint)
        if not (legacy_block or engine_changed or policy_changed):
            return None
        ...
        if branch != str(record.get("featureBranch", "")):
            return PipelineOutcome(
                "VALIDATION_FAILED",
                tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {"branch": branch})]),
                record,
                violations=(_violation("VALIDATION_RECOVERY_REOPEN_BRANCH_MISMATCH", "validation recovery reopen requires the expected feature branch", {"expected": str(record.get("featureBranch", "")), "actual": branch}),),
            )
        if status.staged or status.dirty_tracked or status.untracked:
            return PipelineOutcome(
                "VALIDATION_FAILED",
                tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {})]),
                record,
                violations=(_violation("VALIDATION_RECOVERY_REOPEN_WORKTREE_DIRTY", "validation recovery reopen requires a clean feature worktree", {"staged": ",".join(status.staged), "dirty": ",".join(status.dirty_tracked), "untracked": ",".join(status.untracked)}),),
            )
        if current_head != candidate.candidate_sha:
            return PipelineOutcome(
                "VALIDATION_FAILED",
                tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {"current_head": current_head})]),
                record,
                violations=(_violation("VALIDATION_RECOVERY_REOPEN_HEAD_MISMATCH", "validation recovery reopen requires current HEAD to match failed candidate evidence", {"candidate_sha": candidate.candidate_sha, "current_head": current_head}),),
            )
        ...
        try:
            if not self.git.is_ancestor(worktree, str(record.get("authoritativeBaseSha", "")), current_head):
                return PipelineOutcome(
                    ...
                    tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {"current_head": current_head})]),
                    record,
                    violations=(_violation("VALIDATION_RECOVERY_REOPEN_BASE_STALE", "validation recovery reopen requires candidate to descend from authoritative base", {"base": str(record.get("authoritativeBaseSha", "")), "current_head": current_head}),),
                )
        except RepositoryProviderError as exc:
            return PipelineOutcome("VALIDATION_FAILED", tuple([*stages, _stage("validation_recovery_reopen", "BLOCKED", {})]), record, violations=(_violation(exc.code, exc.message, {"worktree": str(worktree)}),))

        reason = "legacy_exhausted_block_without_runtime_identity" if legacy_block else "recovery_runtime_or_policy_changed"
        reopen = {
            "round": reopen_number,
            "maxReopens": max_reopens,
            "status": "REOPENED",
            ...
            "previousRecoveryAttempts": attempts,
            "previousRecoveryAttemptCount": len(attempts),
            "adosRuntimeBefore": previous_identity if isinstance(previous_identity, dict) else {"gitHead": "unknown", "source": "legacy_block_without_runtime_identity"},
            "adosRuntimeAfter": current_identity,
            "policyFingerprintBefore": previous_policy_fingerprint or "unknown",
            "policyFingerprintAfter": current_policy_fingerprint,
            "newRecoveryBudget": config.execution_policy.validation.max_recovery_rounds,
        }
        reopens_raw = record.get("validationRecoveryReopens")
        reopens = list(reopens_raw) if isinstance(reopens_raw, list) else []
        reopens.append(reopen)
        ...
        updated["validationRecoveryReopens"] = reopens
        updated["validationRecoveryAttempts"] = []
        updated["validationRecoveryReopen"] = reopen
        updated.pop("validationRecoveryBlock", None)
        _write_json(run_record_path.with_name(f"validation-recovery-reopen-{reopen_number}.json"), reopen)
        _write_json(run_record_path, updated)
    ...
    def _reopen_review_side_effect_recovery(
        ...
        evidence_violations = review_side_effect_recovery_reopen_evidence(record, candidate_artifact, validation_artifact, review_artifact)
        if evidence_violations:
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"reason": "not_eligible"})]),
                record,
        ...
        if candidate is None or validation is None or review is None:
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"reason": "invalid_evidence"})]),
                record,
        ...
        if missing:
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"reason": "record_invalid"})]),
                record,
                violations=tuple(_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_RECORD_INVALID", "review side-effect recovery reopen requires durable run identity fields", {"field": key}) for key in missing),
            )
        if not worktree.is_dir():
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {})]),
                record,
                violations=(_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_WORKTREE_MISSING", "review side-effect recovery reopen requires the recorded feature worktree", {"worktree": str(worktree)}),),
            )
        try:
            status = self.git.status(worktree)
            current_head = self.git.current_head(worktree)
        ...
        if branch != str(record.get("featureBranch", "")):
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"branch": branch})]),
                record,
                violations=(_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_BRANCH_MISMATCH", "review side-effect recovery reopen requires the recorded feature branch", {"expected": str(record.get("featureBranch", "")), "actual": branch}),),
            )
        if status.staged or status.dirty_tracked or status.untracked:
            return PipelineOutcome(
                "REVIEW_BLOCKED",
        ...
        try:
            ...
            if not self.git.is_ancestor(worktree, candidate.candidate_sha, current_head):
                return PipelineOutcome(
                    ...
                    tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"current_head": current_head})]),
                    record,
                    violations=(_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_LINEAGE_MISMATCH", "review side-effect recovery reopen requires current HEAD to descend from the blocked reviewed candidate", {"candidate_sha": candidate.candidate_sha, "current_head": current_head}),),
                )
        except RepositoryProviderError as exc:
            return PipelineOutcome("REVIEW_BLOCKED", tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {})]), record, violations=(_violation(exc.code, exc.message, {"worktree": str(worktree)}),))
        ...
        if any(str(item.get("adoptedCandidateSha", "")) == current_head for item in existing_reopens):
            return PipelineOutcome(
                ...
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"current_head": current_head})]),
                record,
                violations=(_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_ALREADY_USED", "review side-effect recovery reopen was already used for this HEAD", {"current_head": current_head}),),
            )
        ...
        reopen = {
            ...
            "featureWorktree": str(worktree),
            "authoritativeBaseSha": str(record.get("authoritativeBaseSha", "")),
            "previousCandidateSha": candidate.candidate_sha,
    ...
    def _restore_failed_review_routing_state(
        self,
        config: ProjectConfig,
        run_record_path: Path,
        record: dict[str, Any],
        ...
        violations = failed_review_routing_state_restoration_evidence(
            self.git,
            run_record_path,
            record,
            candidate_raw,
        ...
        if violations:
            return PipelineOutcome(
                "IMPLEMENTATION_FAILED",
                tuple([*stages, _stage("failed_review_routing_state_restoration", "BLOCKED", {})]),
                record,
        ...
        restoration = {
            ...
            "restoredStatus": "REVIEW_BLOCKED",
            "restoredNextStage": "implementation_recovery",
            "previousStatus": str(record.get("status", "")),
            "previousNextStage": str(record.get("nextStage", "")),
            "previousImplementationRecoveryBlock": prior_block,
            "sourceReviewSideEffectRecoveryReopen": record.get("reviewSideEffectRecoveryReopen"),
        }
        updated = dict(record)
        updated["failedReviewRoutingStateRestoration"] = restoration
        updated.pop("implementationRecoveryBlock", None)
        _write_json(run_record_path.with_name("failed-review-routing-state-restoration.json"), restoration)
        _write_review_block_status(run_record_path, updated, review, candidate, validation)
        restored = _read_json(run_record_path) or updated
        violation = _violation(
            "REVIEW_CHANGES_REQUESTED",
        ...
        return PipelineOutcome(
            "REVIEW_BLOCKED",
            tuple([*stages, _stage("failed_review_routing_state_restoration", "PASS", {"candidate_sha": candidate.candidate_sha})]),
            restored,
            candidate=candidate,
            validation=validation,
            review=review,
    ...
    def _resume_review(
        ...
        *,
        ...
        if current_head != candidate.candidate_sha and not side_effect_resume and not review_side_effect_recovery_reopen_evidence(record, candidate_raw, validation_raw, review_raw):
            block = record.get("reviewBlock")
            max_rounds = _positive_int_from_mapping(block.get("evidence"), "max_rounds") if isinstance(block, dict) else None
            return PipelineOutcome(
                "REVIEW_BLOCKED",
                tuple([*stages, _stage("review_side_effect_recovery_reopen", "BLOCKED", {"reason": "explicit_reopen_required"})]),
                record,
                candidate=candidate,
                validation=validation,
                violations=(
                    _violation(
                        "REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_REQUIRED",
                        "exhausted review side-effect recovery requires explicit human reopen",
                        {"candidate_sha": candidate.candidate_sha, "max_rounds": str(max_rounds or "")},
                    ),
                ),
            )
        if status.staged or status.dirty_tracked or status.untracked:
...
def _validation_recovery_reopen_count(record: dict[str, Any]) -> int:
    reopens = record.get("validationRecoveryReopens")
    return len(reopens) if isinstance(reopens, list) else 0
...
def _write_validation_recovery_block_status(path: Path, record: dict[str, Any], violation: PipelineViolation) -> None:
    ...
    updated["validationRecoveryBlock"] = {
        ...
        "message": violation.message,
        "evidence": violation.evidence,
        "recoveryEngine": _ados_runtime_identity(),
        "policyFingerprint": str(violation.evidence.get("policy_fingerprint", "")),
    }
    _write_json(path, updated)
...
def _review_side_effect_recovery_reopens(record: dict[str, Any]) -> list[dict[str, Any]]:
    reopens = record.get("reviewSideEffectRecoveryReopens")
    if not isinstance(reopens, list):
        return []
    return [dict(item) for item in reopens if isinstance(item, dict)]


def _review_side_effect_recovery_reopen_count(record: dict[str, Any]) -> int:
    return len(_review_side_effect_recovery_reopens(record))
...
def review_side_effect_recovery_reopen_evidence(record: Any, candidate: Any, validation: Any, review: Any) -> tuple[PipelineViolation, ...]:
    if not isinstance(record, dict) or str(record.get("status", "")) != "REVIEW_BLOCKED":
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_RECORD_INVALID", "review side-effect recovery reopen requires REVIEW_BLOCKED durable state", {"status": str(record.get("status", "")) if isinstance(record, dict) else ""}),)
    block = record.get("reviewBlock")
    if not isinstance(block, dict):
    ...
    if reason_codes not in ([], ["REVIEW_SIDE_EFFECT_RECOVERY_MAX_ROUNDS_EXCEEDED"]):
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_REASON_CODES_UNSAFE", "review side-effect recovery reopen requires only exhausted side-effect recovery reason codes", {}),)
    if str(block.get("exitCode", "0")) != "0" or str(block.get("timedOut", "False")) != "False" or str(block.get("transient", "False")) != "False":
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_PROCESS_UNSAFE", "review side-effect recovery reopen requires successful non-transient reviewer process evidence", {}),)

    candidate_result = _candidate_from_mapping(candidate)
    if candidate_result is None or candidate_result.status != "COMMITTED":
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_CANDIDATE_INVALID", "review side-effect recovery reopen requires committed candidate evidence", {}),)
    if not isinstance(validation, dict):
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_VALIDATION_MISSING", "review side-effect recovery reopen requires validation evidence", {}),)
    validation_result = _validation_from_mapping(validation)
    if validation_result.status != "PASS":
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_VALIDATION_NOT_PASSED", "review side-effect recovery reopen requires previously passed validation", {"status": validation_result.status}),)
    if validation_result.head_before != candidate_result.candidate_sha or validation_result.head_after != candidate_result.candidate_sha:
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_VALIDATION_SHA_MISMATCH", "review side-effect recovery reopen validation evidence does not match blocked candidate", {"candidate_sha": candidate_result.candidate_sha, "head_before": validation_result.head_before, "head_after": validation_result.head_after}),)
    if not isinstance(review, dict):
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_REVIEW_MISSING", "review side-effect recovery reopen requires review evidence", {}),)
    review_result = _review_from_mapping(review)
    if review_result.status != "PASS" or review_result.reviewed_sha != candidate_result.candidate_sha:
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_REVIEW_UNSAFE", "review side-effect recovery reopen requires successful review evidence for the blocked candidate", {"status": review_result.status, "reviewed_sha": review_result.reviewed_sha, "candidate_sha": candidate_result.candidate_sha}),)
    if str(block.get("candidateSha", "")) != candidate_result.candidate_sha or str(block.get("validatedSha", "")) != validation_result.head_after or str(block.get("reviewedSha", "")) != review_result.reviewed_sha:
        return (_violation("REVIEW_SIDE_EFFECT_RECOVERY_REOPEN_BLOCK_SHA_MISMATCH", "review side-effect recovery reopen durable block SHA evidence does not match artifacts", {"candidate_sha": candidate_result.candidate_sha, "validated_sha": validation_result.head_after, "reviewed_sha": review_result.reviewed_sha}),)

    attempts = _review_side_effect_recovery_attempts(record)
    cycle_key = _review_side_effect_recovery_cycle_key(candidate_result, validation_result, review_result)
    cycle_attempts = _review_side_effect_recovery_attempts_for_cycle(attempts, cycle_key, candidate_result, validation_result, review_result)
    max_rounds = _positive_int_from_mapping(block.get("evidence"), "max_rounds") or _positive_int_from_mapping(block.get("evidence"), "max_recovery_rounds")
    if max_rounds is None:
        max_rounds = _positive_int_from_mapping(block, "maxRounds")
    if max_rounds is None and cycle_attempts:
        max_rounds = _positive_int_from_mapping(cycle_attempts[-1], "maxRounds")
    if max_rounds is None:
...
def failed_review_routing_state_restoration_evidence(
    ...
    if not isinstance(record, dict):
        return (_violation("FAILED_REVIEW_ROUTING_RESTORE_RECORD_INVALID", "routing-state restoration requires a durable run record", {}),)
    if str(record.get("status", "")) != "IMPLEMENTATION_FAILED" or str(record.get("nextStage", "")) != "human_intervention":
        reject("FAILED_REVIEW_ROUTING_RESTORE_STATE_INVALID", "routing-state restoration requires the exact implementation failure state", {"status": str(record.get("status", "")), "next_stage": str(record.get("nextStage", ""))})
    if "failedReviewRoutingStateRestoration" in record:
        reject("FAILED_REVIEW_ROUTING_RESTORE_ALREADY_APPLIED", "routing-state restoration may be applied only once")
    if "reviewBlock" in record or "implementationFailure" in record:
        reject("FAILED_REVIEW_ROUTING_RESTORE_CONFLICTING_STATE", "routing-state restoration rejects conflicting current failure evidence")

    block = record.get("implementationRecoveryBlock")
    block_evidence = block.get("evidence") if isinstance(block, dict) else None
    if not isinstance(block, dict) or str(block.get("status", "")) != "BLOCKED" or str(block.get("reasonCode", "")) != "FAILOVER_CANDIDATE_ALREADY_PRODUCED":
        reject("FAILED_REVIEW_ROUTING_RESTORE_FAILOVER_BLOCK_INVALID", "routing-state restoration requires the exact candidate-preserving failover block")
    if not isinstance(block_evidence, dict) or str(block_evidence.get("category", "")) != "TRANSIENT_RUNTIME_UNAVAILABLE":
        reject("FAILED_REVIEW_ROUTING_RESTORE_FAILOVER_CATEGORY_INVALID", "routing-state restoration requires transient runtime-unavailable failover evidence")
    ...
    try:
        ...
        review_result = None
        reject("FAILED_REVIEW_ROUTING_RESTORE_ARTIFACT_MALFORMED", "routing-state restoration requires well-formed validation and review artifacts")
    if candidate_result is None or candidate_result.status != "COMMITTED":
        reject("FAILED_REVIEW_ROUTING_RESTORE_CANDIDATE_INVALID", "routing-state restoration requires committed candidate evidence")
    if validation_result is None or validation_result.status != "PASS":
        reject("FAILED_REVIEW_ROUTING_RESTORE_VALIDATION_INVALID", "routing-state restoration requires passed validation evidence")
    if review_result is None or review_result.status != "PASS" or review_result.decision != "Changes Requested" or review_result.exit_code != 0:
        reject("FAILED_REVIEW_ROUTING_RESTORE_REVIEW_INVALID", "routing-state restoration requires a successful Changes Requested review")

    exact_sha = candidate_result.candidate_sha if candidate_result is not None else ""
    if not exact_sha:
        reject("FAILED_REVIEW_ROUTING_RESTORE_SHA_MISSING", "routing-state restoration requires an exact candidate SHA")
    if validation_result is not None and (validation_result.head_before != exact_sha or validation_result.head_after != exact_sha):
        reject("FAILED_REVIEW_ROUTING_RESTORE_VALIDATION_SHA_MISMATCH", "validation evidence must match the exact candidate", {"candidate_sha": exact_sha, "head_before": validation_result.head_before, "head_after": validation_result.head_after})
    if review_result is not None and review_result.reviewed_sha != exact_sha:
        reject("FAILED_REVIEW_ROUTING_RESTORE_REVIEW_SHA_MISMATCH", "review evidence must match the exact candidate", {"candidate_sha": exact_sha, "reviewed_sha": review_result.reviewed_sha})
    if isinstance(block_evidence, dict) and (
        str(block_evidence.get("head_before", "")) != exact_sha
        or str(block_evidence.get("head_after", "")) != exact_sha
        or str(block_evidence.get("changed_files", "")) != ""
        or str(block_evidence.get("base_sha", "")) != str(record.get("authoritativeBaseSha", ""))
    ):
        reject("FAILED_REVIEW_ROUTING_RESTORE_FAILOVER_EVIDENCE_MISMATCH", "failover block must prove an unchanged exact candidate")

    reopen = record.get("reviewSideEffectRecoveryReopen")
    authorization = record.get("reviewSideEffectRecoveryReopenAuthorization")
    if not isinstance(reopen, dict) or str(reopen.get("status", "")) != "REOPENED" or str(reopen.get("mode", "")) != "failed_reviewer_dirty_side_effect" or str(reopen.get("selectedNextStage", "")) != "review":
        reject("FAILED_REVIEW_ROUTING_RESTORE_REOPEN_INVALID", "routing-state restoration requires the prior failed-review side-effect reopen")
    if not isinstance(authorization, dict) or str(authorization.get("status", "")) != "AUTHORIZED" or str(authorization.get("mode", "")) != "failed_reviewer_dirty_side_effect":
        reject("FAILED_REVIEW_ROUTING_RESTORE_AUTHORIZATION_INVALID", "routing-state restoration requires failed-review reopen authorization")

    requirements_sha = str(record.get("requirements", {}).get("sha256", "")) if isinstance(record.get("requirements"), dict) else ""
    assignment = record.get("agentAssignment") if isinstance(record.get("agentAssignment"), dict) else None
    assignment_sequence = _positive_int_from_mapping(assignment, "sequence") if assignment is not None else None
    if not requirements_sha:
        reject("FAILED_REVIEW_ROUTING_RESTORE_REQUIREMENTS_MISSING", "routing-state restoration requires durable requirements identity")
    if isinstance(reopen, dict):
        if str(reopen.get("runId", "")) != str(record.get("runId", "")) or str(reopen.get("featureBranch", "")) != str(record.get("featureBranch", "")) or Path(str(reopen.get("featureWorktree", ""))).resolve() != Path(str(record.get("featureWorktree", ""))).resolve():
            reject("FAILED_REVIEW_ROUTING_RESTORE_RUN_IDENTITY_MISMATCH", "reopen evidence must match the durable run, branch, and worktree")
        if str(reopen.get("requirementsSha", "")) != requirements_sha:
            reject("FAILED_REVIEW_ROUTING_RESTORE_REQUIREMENTS_MISMATCH", "reopen requirements identity must match the durable run")
        if str(reopen.get("pinnedImplementer", "")) != str(record.get("implementer", "")) or str(reopen.get("pinnedReviewer", "")) != str(record.get("reviewer", "")) or reopen.get("pinnedAgentAssignment") != assignment:
            reject("FAILED_REVIEW_ROUTING_RESTORE_ROLES_MISMATCH", "reopen role assignment must match the durable run")
        if str(reopen.get("adoptedCandidateSha", "")) != exact_sha:
            reject("FAILED_REVIEW_ROUTING_RESTORE_REOPEN_SHA_MISMATCH", "reopen evidence must identify the exact candidate")
    if isinstance(authorization, dict):
        if str(authorization.get("runId", "")) != str(record.get("runId", "")) or str(authorization.get("requirementsSha", "")) != requirements_sha:
            reject("FAILED_REVIEW_ROUTING_RESTORE_AUTHORIZATION_IDENTITY_MISMATCH", "reopen authorization must match run and requirements identity")
        if any(str(authorization.get(key, "")) != exact_sha for key in ("candidateSha", "validatedSha", "reviewedSha")):
            reject("FAILED_REVIEW_ROUTING_RESTORE_AUTHORIZATION_SHA_MISMATCH", "reopen authorization must remain exact-SHA bound")
        if isinstance(reopen, dict) and _positive_int_from_mapping(authorization, "reopenRound") != _positive_int_from_mapping(reopen, "round"):
            reject("FAILED_REVIEW_ROUTING_RESTORE_AUTHORIZATION_ROUND_MISMATCH", "reopen authorization must identify the same reopen round")
    if assignment is None or assignment_sequence is None or not str(assignment.get("candidateOwnerId", "")):
        reject("FAILED_REVIEW_ROUTING_RESTORE_ASSIGNMENT_INVALID", "routing-state restoration requires a pinned candidate owner and assignment sequence")
    elif str(assignment.get("implementerCommand", "")) != str(record.get("implementer", "")) or str(assignment.get("reviewerCommand", "")) != str(record.get("reviewer", "")):
        reject("FAILED_REVIEW_ROUTING_RESTORE_ASSIGNMENT_MISMATCH", "durable role commands must match the pinned assignment")

    result = implementer.get("result") if isinstance(implementer, dict) else None
    changed_files = result.get("changedFiles") if isinstance(result, dict) else None
    if not isinstance(result, dict) or str(result.get("status", "")) != "IMPLEMENTATION_FAILED" or _positive_int_from_mapping(result, "exitCode") is None or result.get("timedOut") is not False:
        reject("FAILED_REVIEW_ROUTING_RESTORE_IMPLEMENTER_RUNTIME_INVALID", "routing-state restoration requires the failed implementer runtime artifact")
    if isinstance(result, dict) and (
        ...
        reject("FAILED_REVIEW_ROUTING_RESTORE_IMPLEMENTER_EVIDENCE_MISMATCH", "failed implementer evidence must prove no candidate or worktree change")
    ...
    if assignment is not None and (
        ...
        or _positive_int_from_mapping(last_event, "sequence") != assignment_sequence
    ):
        reject("FAILED_REVIEW_ROUTING_RESTORE_AVAILABILITY_MISMATCH", "latest availability evidence must match the failed pinned implementer")
    ...
    try:
        ...
        if status.root != worktree.resolve() or branch != str(record.get("featureBranch", "")):
            reject("FAILED_REVIEW_ROUTING_RESTORE_WORKTREE_IDENTITY_MISMATCH", "feature worktree and branch must match durable identity")
        if status.staged or status.dirty_tracked or status.untracked:
            reject("FAILED_REVIEW_ROUTING_RESTORE_WORKTREE_DIRTY", "routing-state restoration requires a clean feature worktree", {"staged": ",".join(status.staged), "dirty": ",".join(status.dirty_tracked), "untracked": ",".join(status.untracked)})
        if current_head != exact_sha:
            reject("FAILED_REVIEW_ROUTING_RESTORE_HEAD_MISMATCH", "current HEAD must match the exact candidate", {"candidate_sha": exact_sha, "current_head": current_head})
    ...
    try:
        artifacts_present = all(path.is_file() for path in artifact_paths)
        implementer_time = artifact_paths[-1].stat().st_mtime_ns if artifacts_present else 0
        newer_evidence = artifacts_present and any(path.stat().st_mtime_ns > implementer_time for path in artifact_paths[:-1])
    except OSError:
        artifacts_present = False
        newer_evidence = False
...
def _policy_fingerprint(config: ProjectConfig) -> str:
    return hashlib.sha256(config.execution_policy.to_json().encode("utf-8")).hexdigest()


def _ados_runtime_identity() -> dict[str, str]:
    root = Path(__file__).resolve().parent.parent
    completed = _run(("git", "rev-parse", "HEAD"), root)
    git_head = completed.stdout.strip() if completed.returncode == 0 else "unknown"
    return {"source": "ados", "gitHead": git_head}


def _positive_int_from_mapping(raw: Any, key: str) -> int | None:
    if not isinstance(raw, dict):
        return None
    try:
